...
\section{Опыт}
    \resumeSubHeadingListStart
        ...
        \resumeItemListStart
            ...
            \resumeItem{Рефакторил существующие модули, добавлял новые фичи, оптимизировал запутанную бизнес-логику. При переносе на микросервисы переписывал бóльшую часть кода, т.к. значительно декомпозировали монолит. Маппинг и работу с DAO менял на асинхронные обращения к микросервисам через WebClient. С нуля разработал модули интеграционного тестирования.}
            \resumeItem{Раскатывал сборки в Jenkins, артефакты хранили в Nexus, код проверяли в SonarQube, занимался сборкой релизов, раскатывал новые микросервисы в OpenShift(Kubernetes), писал шедулеры.}
            \resumeItem{Проводил код ревью, дополнял бэклог, участвовал в планировании задач, груминге, демо.}
        ...
        \resumeItemListStart
            ...
            \resumeItem{\texttt{Стек: Java, Spring: Boot, Webflux; Maven, JUnit, Mockito, Jacoco, Redis, Resilience4j, GitLab CI/CD, Docker}}
        \resumeItemListEnd
...
\begin{itemize}[leftmargin=0.15in, label={}, itemsep=-1pt]
    \small{
        \item Java 8, 11, 17, Core, OOP, Collections, Multithreading, Stream API | Spring Core, Boot, MVC, Security, Data, WebFlux | Hibernate ORM | JDBC | PostgreSQL, MySQL, H2 | Criteria API | MapStruct | Maven/Gradle | Sonatype Nexus | Docker | Gitlab CI/CD, Runner | Jenkins | SonarQube | OpenShift | BitBucket | Linux | REST | Swagger | JUnit | Mockito | Bash | Liquibase 
    }
\end{itemize}
\end{document}
